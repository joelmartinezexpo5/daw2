...
import { mostrarError, crearElemento } from '../../js/utils.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.tagName === 'BUTTON' && e.target.dataset.accion) {
      ...
      // Ajusta según tus nombres de archivos HTML
      const paginasAccion = {
        'todos': 'tareas.html',
        'albums': 'albumes.html',
        'posts': 'publicaciones.html'
      };
      window.location.href = `${paginasAccion[accion]}?userId=${userId}`;
    }
  });
});